...
namespace trklet {
  ...
  void AnalyzerTBout::fill(const FrameTrack& frameTrack, const FrameStub& frameStub) {
    ...
    const bool psModule = setup_->psModule(frameStub.first);
    ...
    //const double basePhi = setup_->hybridBasePhi(type);
    const double basePhi = settings_.kphi1();
    const double baseZ = setup_->hybridBaseZ(type);
    ...
    const double phid = (floor(phi / basePhi) + .5) * basePhi;
    const double rzd = (floor(rz / baseRZ) + .5) * baseRZ;
    // parse residuals
