...
void PurgeDuplicate::execute(std::vector<Track>& outputtracks_, unsigned int iSector) {
  ...
  if (settings_.removalType() == "merge") {
    ...
    /*std::ofstream CountMap;
    std::ofstream Comp;
    Comp.open("Comp.txt");
    CountMap.open("CountMap.txt");
    for (unsigned int itrk = 0; itrk < numStublists; itrk++) {
      for (unsigned int jtrk=0; jtrk < numStublists; jtrk++) {
        CountMap<<dupMapCount[itrk][jtrk];
      }
      Comp<<dupComp[itrk]; 
    }
    Comp.close();
    ...
    */
    ...
    for (unsigned int itrk = 0; itrk < numStublists; itrk++) {
      ...
      if (duplicateTrack) {
        if (mergeCount[itrk] >= 1) {
          int reconCount = 0;
          std::vector<int> compTrack;
          for (unsigned int jtrk = 0; jtrk < numStublists; jtrk++) {
            if (dupMap[itrk][jtrk]) {
              compTrack.push_back(jtrk);
            }
          }
          for (unsigned int ktrk = 0; ktrk < compTrack.size()-1; ktrk++) {
            for (unsigned int mtrk = ktrk+1; mtrk < compTrack.size(); mtrk++) { 
              Tracklet* track1 = inputtracklets_[ktrk];
              Tracklet* track2 = inputtracklets_[mtrk];
              if (shareOverlapRInvBins(findOverlapRInvBins(track1), findOverlapRInvBins(track2)) == 0) {
                reconCount += 1;
              }
            }
          }
